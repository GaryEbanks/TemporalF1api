...
class MainActivity : AppCompatActivity() {
    ...
    fun getRetrofit(): Retrofit{
        return Retrofit
            .Builder()
            .baseUrl("https://ergast.com/api/f1/2022/drivers.json")
            .client(OkHttpClient())
    ...
    fun searchDriverData(){
        CoroutineScope(Dispatchers.Main).launch {
            try {
                val txtFilter: String = binding.etDriverName.text.toString()
                val call = getRetrofit().create(ApiService::class.java).getDriverData("/$txtFilter.json")
                if (call.isSuccessful){
                    val givenName: String = call.body()?.givenName.toString()
                    val familyName: String = call.body()?.familyName.toString()
                    val dateOfBirth: String = call.body()?.dateOfBirth.toString()
                    val nationality: String = call.body()?.nationality.toString()
                    val number: String = call.body()?.permanentNumber.toString()
                    val code: String = call.body()?.code.toString()
